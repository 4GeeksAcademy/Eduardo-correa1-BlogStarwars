  ...
  const {planets} = store
    return (
      ...
      <div className="carousel mt-3 mb-3">
            {chars.map((character) => (
                <Card key={character.uid} character={character} imageUrl={imagecharacters[character.uid]} />
            ))}
        </div>
      <div className="carousel mt-3 mb-3">
      {planets.map((character) => (
          <Card key={character.uid} character={character} imageUrl={imageplanets[character.uid]} />
      ))}
  </div>
  </div>
